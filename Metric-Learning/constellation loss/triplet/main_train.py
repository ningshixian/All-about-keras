...
class PlotLosses(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        plt.savefig('final_exp/' + str(BATCH_SIZE//32) + '/plots/inception_triplet_' + str(fold) + '.png');
...
if __name__ == '__main__':
    ...
    # PARAMETERS
    BATCH_LIST = [32*i for i in range(1,8)] #Try different batches, from 32 to 32 * 7
    IMG_SIZE = 150
    EMB_VECTOR = 128
    EPOCHS = 10
    ...
    # Same seeds for all the experiments
        ...
        for seed in random_seeds:
            ...
            if keras.backend.backend() == 'tensorflow':
                ...
                with K.tf.device(gpu_device):
                    ...
                    model.compile(loss=batch_all_triplet_loss, optimizer=keras.optimizers.Adam(1e-3))

                    model.fit_generator(generator(x_train, y_train, BATCH_SIZE), steps_per_epoch=steps_train,
                                        validation_data=generator(x_test, y_test, BATCH_SIZE), validation_steps=steps_test,
                                        epochs=EPOCHS, callbacks=[loss_plot])

                    model.save_weights('final_exp/' + str(BATCH_SIZE//32) + '/weights/inception_triplet_fold_' + str(fold) + '.h5')
